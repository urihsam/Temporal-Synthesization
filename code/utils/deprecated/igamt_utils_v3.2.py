...
def model_evaluation(args, models, opts, lrs, data_loader, prob_mask, split, log_file, **kwargs):
    ...
    input_match_total_loss, gen_match_total_loss = 0.0, 0.0
    input_m_match_total_loss, gen_m_match_total_loss = 0.0, 0.0
    ...
    for iteration, batch in enumerate(data_loader):
        #
        match_loss, gen_match_loss = 0.0, 0.0
        m_match_loss, gen_m_match_loss = 0.0, 0.0
        #
        ...
        if split == 'train':
            ...
            opt_imi.zero_grad()
            # match
            #gen_match_loss = args.beta_match*Imi.compute_recon_loss(Pimi, Pgen, Mimi, Mgen, type="mse")
            gen_match_loss.backward(inputs=params, retain_graph=True)
            # mask match
            #gen_m_match_loss = args.beta_match*Imi.compute_mask_loss(Mimi, Mgen, type="mse")
        ...
        if split == 'train' and iteration % args.train_eval_freq == 0:
            ...
            print("Batch loss:")
            print("%s\trecon_loss\t%9.4f\tmask_loss\t%9.4f\txCritic_loss\t%9.4f\tzCritic_loss\t%9.4f\tmCritic_loss\t%9.4f\n\tinput_match_loss\t%9.4f\tgen_match_loss\t%9.4f\tinput_mask_match_loss\t%9.4f\timi_mask_match_loss\t%9.4f"%(
                    ...
                    input_match_loss/batch_size, gen_match_loss/batch_size, input_m_match_loss/batch_size, gen_m_match_loss/batch_size))
            ...
            print("%s\trecon_loss\t%9.4f\tmask_loss\t%9.4f\txCritic_loss\t%9.4f\tzCritic_loss\t%9.4f\tmCritic_loss\t%9.4f\n\tinput_match_loss\t%9.4f\tgen_match_loss\t%9.4f\tinput_mask_match_loss\t%9.4f\timi_mask_match_loss\t%9.4f"%(
                    ...
                    input_match_total_loss/n_data, gen_match_total_loss/n_data, input_m_match_total_loss/n_data, gen_m_match_total_loss/n_data))
            print()
