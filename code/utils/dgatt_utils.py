...
def model_evaluation(args, models, opts, lrs, data_loader, prob_mask, split, log_file):
    ...
    for iteration, batch in enumerate(data_loader):
        ...
        if args.no_mask:
            z, Pinput, Poutput, Moutput = Trans(batch['src_tempo'], batch['tgt_tempo'], None, None, **kwargs)
            # loss
            recon_loss = args.beta_recon * Trans.compute_recon_loss(Poutput, batch['tgt_tempo'], None, None)
        elif args.use_prob_mask:
            z, Pinput, Poutput, Moutput = Trans(batch['src_tempo'], batch['tgt_tempo'], batch['src_mask'], batch['tgt_mask'], **kwargs)
