...
def position_encoding(seq_len: int, dim_model: int) -> Tensor:
    ...
    return torch.where(dim.long() % 2 == 0, torch.sin(phase), torch.cos(phase)).cuda()
...
class TransformerDecoder(torch.nn.Module):
    ...
    def forward(self, tgt: Tensor, memory: Tensor, tgt_mask: Tensor = None) -> Tensor:
        ...
        output = torch.nn.functional.leaky_relu(self.linear(tgt), 0.8)
    ...
    def inference(self, start_feature: Tensor, z: Tensor, start_mask: Tensor = None, prob_mask: Tensor = None):
        ...
        while(t+1<self.max_length and len(running_seqs)>0):
            ...
            seq_len, dimension = input_.size(1), input_.size(2)
            input_ += position_encoding(seq_len, dimension)
            for layer in self.layers:
                input_ = layer(input_, z_)


            input_sequence = torch.nn.functional.leaky_relu(self.linear(input_), 0.8)
